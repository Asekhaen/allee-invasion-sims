...
m.vec <- seq(0.05, 0.45, 0.05)
r.vec <- mean(c(24, 23, 13.5)) # from Izzy's experiment: dating + 1 day max growth in DF10 lines
K.vec <- c(30, 50, 100)
n.vec <- seq(10, 50, 10)

p.space <- expand.grid(exp.reps = n.vec, m = m.vec, r = r.vec, K = K.vec)
...
# combine results
p.space <- cbind(p.space, 
                 e.size = -apply(e.size, 1, mean, na.rm = TRUE), # switch orientation for easier intuition
                 p.value = apply(p.value, 1, mean, na.rm = TRUE),
                 power = apply(p.value<0.05, 1, mean, na.rm = TRUE))
...
library(ggplot2)
# power at K = 50, n = 10
ggplot(p.space, aes(x = exp.reps, y = power)) +
  geom_line() +
  facet_grid(rows = vars(m), cols = vars(K)) +
  labs(title = "Power for varying K, and m") +
  xlab("Number of replicate invasions") +
  ylab("Power") +
  theme_minimal()
ggsave(filename = "out/power-k-m-var_n.png")

ggplot(p.space, aes(x = exp.reps, y = e.size)) +
  geom_line() +
  facet_grid(rows = vars(m), cols = vars(K)) +
  labs(title = "Effect size for varying K, and m") +
  xlab("Number of replicate invasions") +
  ylab("Effect size") +
  theme_minimal()
ggsave(filename = "out/esize-k-m-var_n.png")
